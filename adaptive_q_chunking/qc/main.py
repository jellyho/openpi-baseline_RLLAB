...
flags.DEFINE_string('wandb_entity', None, 'W&B entity (team/org). None uses your account default.')
flags.DEFINE_string('wandb_project', 'AQC', 'W&B project name.')
...
# Distributional value support (only used when --agent.critic_type=distributional).
flags.DEFINE_string('support_type', 'data',
...
flags.DEFINE_bool('aqc_diagnostic_video', True,
                  'For AQC agents: also log a composite rollout+prefix-Q+chunk-size diagnostic video at eval.')
...
def maybe_log_aqc_video(agent, eval_env, action_dim, log_step):
    """For AQC agents, log a composite rollout + prefix-Q + chunk-size diagnostic video.

    Best-effort: any failure (e.g. rendering issue) is logged and skipped so that it never
    interrupts training. No-op for non-AQC agents or when video logging is disabled.
    """
    if not FLAGS.aqc_diagnostic_video or FLAGS.video_episodes <= 0:
        return
    if not hasattr(agent, 'sample_actions_with_info'):
        return
    try:
        from utils.aqc_viz import aqc_rollout_video
        rng = jax.random.PRNGKey(np.random.randint(0, 2 ** 31 - 1))
        video, vstats = aqc_rollout_video(
            agent, eval_env, action_dim=action_dim, horizon=FLAGS.horizon_length,
            rng=rng, num_episodes=1, frame_skip=FLAGS.video_frame_skip,
        )
        if video is not None:
            wandb.log({
                'eval/aqc_video': wandb.Video(video, fps=15, format='mp4'),
                'eval/aqc_video_mean_h': vstats['mean_h'],
            }, step=log_step)
    except Exception as e:
        print(f'[aqc viz] skipped diagnostic video: {e}', flush=True)

def main(_):
    exp_name = get_exp_name(FLAGS.seed)
    run = setup_wandb(entity=FLAGS.wandb_entity, project=FLAGS.wandb_project,
    ...
    # Initial eval at step 0 (untrained baseline reference, before any gradient step).
    if FLAGS.offline_steps > 0 and FLAGS.eval_interval != 0:
        eval_info, _, _ = evaluate(
            agent=agent,
            env=eval_env,
            action_dim=example_batch["actions"].shape[-1],
            num_eval_episodes=FLAGS.eval_episodes,
            num_video_episodes=0,  # skip video for the untrained baseline
            video_frame_skip=FLAGS.video_frame_skip,
        )
        logger.log(eval_info, "eval", step=log_step)  # log_step == 0 here
    ...
    for i in tqdm.tqdm(range(1, FLAGS.offline_steps + 1)):
        ...
        if i == FLAGS.offline_steps - 1 or \
            ...
            if FLAGS.video_episodes > 0 and len(renders) > 0:
                wandb.log({'eval/video': get_wandb_video(renders=renders)}, step=log_step)
            maybe_log_aqc_video(agent, eval_env, example_batch["actions"].shape[-1], log_step)
    ...
    for i in tqdm.tqdm(range(1, FLAGS.online_steps + 1)):
        ...
        # during online rl, the (possibly adaptive-length) action chunk is executed fully
        if len(action_queue) == 0:
            result = agent.sample_actions(observations=ob, rng=key)
            if isinstance(result, tuple):
                # Adaptive Q-Chunking: (full_chunk, h_star); execute the selected prefix.
                chunk, h_star = result
                action_chunk = np.array(chunk).reshape(-1, action_dim)[: int(h_star)]
            else:
                action_chunk = np.array(result).reshape(-1, action_dim)
        ...
        if i == FLAGS.online_steps - 1 or \
            ...
            if FLAGS.video_episodes > 0 and len(renders) > 0:
                wandb.log({'eval/video': get_wandb_video(renders=renders)}, step=log_step)
            maybe_log_aqc_video(agent, eval_env, action_dim, log_step)
