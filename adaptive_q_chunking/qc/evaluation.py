...
def evaluate(
    ...
    for i in trange(num_eval_episodes + num_video_episodes):
        ...
        step = 0
        render = []
        chunk_lengths = []  # executed chunk size per replanning step (adaptive for AQC)
        ...
        while not done:
            ...
            if len(action_queue) == 0:
                result = actor_fn(observations=observation)
                if isinstance(result, tuple):
                    # Adaptive Q-Chunking returns (full_chunk, h_star); execute the
                    # selected prefix and replan at the chunk boundary.
                    chunk, h_star = result
                    chunk = np.array(chunk).reshape(-1, action_dim)[: int(h_star)]
                else:
                    # Fixed-chunk agents (QC, etc.) return the chunk directly.
                    chunk = np.array(result).reshape(-1, action_dim)
        ...
        # Executed chunk-size statistics (adaptive horizon for AQC; constant for QC).
        if len(chunk_lengths) > 0:
            add_to(stats, {
                "chunk_len_mean": float(np.mean(chunk_lengths)),
                "chunk_len_min": float(np.min(chunk_lengths)),
                "chunk_len_max": float(np.max(chunk_lengths)),
            })
