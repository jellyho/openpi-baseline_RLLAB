...
# robomimic is only needed for the robomimic benchmark; it is an optional dependency.
# OGBench (and the AQC experiments) never use it, so make the import lazy/optional so that
# `main.py` can run on environments where robomimic is not installed (e.g. the `deas_real`
# env). Functions that actually require robomimic raise a clear error if called without it.
try:
    import robomimic.utils.env_utils as EnvUtils
    import robomimic.utils.file_utils as FileUtils
    import robomimic.utils.obs_utils as ObsUtils
    from robomimic import DATASET_REGISTRY
    _ROBOMIMIC_AVAILABLE = True
except ImportError:
    EnvUtils = FileUtils = ObsUtils = DATASET_REGISTRY = None
    _ROBOMIMIC_AVAILABLE = False
